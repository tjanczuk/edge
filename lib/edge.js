...
function normalizeEdgeSpec(options) {
	...
	if (options.sync !== undefined && typeof options.sync !== 'boolean') {
		throw new Error('The sync parameter must be a boolean')
	}

	options.sync = options.sync === undefined ? false : options.sync;

	return options;
}

function getEdgeAppKey(options) {
	return (options.csx || (options.typeName + ', ' + options.methodName + ', ' + options.assemblyFile)) + options.sync;
}

function ensureEdgeApp(options) {
...
exports.func = function(options) {
	...
	var appId = ensureEdgeApp(options);
...
exports.funcSync = function(options) {
	options = normalizeEdgeSpec(options);
	options.sync = true;
	var appId = ensureEdgeApp(options);

	return function (data) {
		return edge.callClrFunc(appId, data);
	};
}
