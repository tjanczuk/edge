...
function wrapJavaScriptFunctionForCLR(func) {
	return function (correlator, payload, callback) {
		try {
			var json = JSON.parse(payload);
			func(json, function (error, result) {
				if (error) {
					callback(correlator, stringifyException(error));
				}
					...
					try {
						...
						return callback(correlator, stringifyException(e1));
					}

					callback(correlator, null, json);
				}
				else {
					callback(correlator);
				}
			...
			callback(correlator, stringifyException(e));
		}
	};
}

function addFunctionsToEnv(options, env) {
	if (typeof options.exports === 'object') {
		for (var i in options.exports) {
			if (typeof options.exports[i] === 'function') {
				if (!env['nodejs.Functions']) {
					env['nodejs.Functions'] = {};
				}
				
				env['nodejs.Functions'][i] = wrapJavaScriptFunctionForCLR(options.exports[i]);
			}
		}
	}
...
module.exports.worker = function (options, input, callback) {
	...
	owin.callOwinApp(appId, addFunctionsToEnv(options, createOwinEnv(input)), createOwinCallback(callback));
};
...
module.exports.sql = function (options, callback) {
	...
	owin.callOwinApp(appId, addFunctionsToEnv(options, createOwinEnv(options)), createOwinCallback(callback));
};
