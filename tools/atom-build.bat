...
SET DESTDIRROOT=%SELF%\..\lib\atom-shell

set VERSIONS=0.11.14
pushd %SELF%\..
...
HOME=%USERPROFILE%\.atom-shell-gyp
"%NODEEXE%" "%GYP%" configure build --msvs_version=2013 --ensure -%FLAVOR% --target=0.16.0 --arch=ia32 --dist-url=https://gh-contractor-zcbenz.s3.amazonaws.com/atom-shell/dist
if %ERRORLEVEL% neq 0 (
